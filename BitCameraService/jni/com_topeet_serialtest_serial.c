...
JNIEXPORT jintArray JNICALL Java_com_topeet_serialtest_serial_Read
  (JNIEnv *env, jobject obj)
 {
		...
		int BufToJavalow[256];

		int BufToJava[256];
		int len = 0, i = 0,j=0,len2=0;
		...
		if(len == 4){
			...
			for(j=0; j<4; j++)
				{
					BufToJava[j] = BufToJavahigh[j];
				}
			for(i=0; i<len; i++)
			{
				//BufToJavahigh[i] = buffer[i];
				//printf("%x", buffer[i]);
				BufToJava[i+4] = buffer[i];
			}
		}

		//__android_log_print(ANDROID_LOG_INFO, "serial", "serial open ok len=%d", len);
		jintArray array = (*env)-> NewIntArray(env, 7);
		(*env)->SetIntArrayRegion(env, array, 0, 7, BufToJava);
...
JNIEXPORT jintArray JNICALL Java_com_topeet_serialtest_serial_Write
  (JNIEnv *env, jobject obj, jintArray buf, jint buflen)
 {
	...
	array = NULL;
	(*env)->ReleaseIntArrayElements(env, buf, body, 0);
	return 0;	
